...
const CardVisualizer: FC<CardVisualizerProps> = ({ room, onCardClick, selectedBedId }) => {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mt-2">
      {room.beds.map((bed) => (
        <div
          key={bed.id}
          className={`p-4 border cursor-pointer rounded-md ${getBackgroundColor({ bed, selectedBedId })}`}
        ...
        >
          ...
          {!bed.isUsed && selectedBedId === bed.id && (
            <p className="text-black font-semibold p-2 rounded-lg underline">Assigned</p>
          )}

          {bed.isUsed && selectedBedId === bed.id && (
            <div className="text-black font-semibold p-2 rounded-lg underline">Patient details</div>
          )}
        </div>
      ))}
    </div>
